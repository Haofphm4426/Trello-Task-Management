...
function ActiveCard() {
  ...
  const isActiveCard = useSelector(selectIsShowModalActiveCard);
  ...
  useEffect(() => {
    const onReceiveNewComments = (updatedCard) => {
      if (updatedCard._id === activeCard?._id) {
  ...
  const handleApiUpdateCard = async (updateCard) => {
    const updatedCard = await updateCardDetailsAPI(activeCard._id, updateCard);
  ...
  const onUpdateCardTitle = (newTitle) => {
    handleApiUpdateCard({ title: newTitle.trim() });
  };

  const onUpdateCardDescription = (newDescription) => {
    handleApiUpdateCard({ description: newDescription });
  };
  ...
  const onUploadCardCover = (e) => {
    ...
    toast.promise(
      handleApiUpdateCard(reqData).finally(() => e.target.value = ''),
      { pending: 'Updating...',
  ...
  //Để async cho onAddCardComment được truyền xuống thằng con có thể .then
  const onAddCardComment = async (commentToAdd) => {
    const updatedCard = await handleApiUpdateCard({ commentToAdd });
    return updatedCard;
  };

  const onUpdateCardMember = (incomingMemberInfo) => {
    handleApiUpdateCard({ incomingMemberInfo });
  };
