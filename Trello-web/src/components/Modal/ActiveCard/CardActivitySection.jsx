...
function CardActivitySection({ cardComments=[], onAddCardComment }) {
  ...
  const handleAddCardComment = (event) => {
    ...
    if (event.key === 'Enter' && !event.shiftKey) {
      ...
      onAddCardComment(commentToAdd).then((updatedCard) => {
        event.target.value = '';
        socketIoInstance.emit('FE_UPDATE_CMT_TO_CARD', updatedCard);
      });
    }
  };
  ...
  return (
    <Box sx={{ mt: 2 }}>
      ...
      {cardComments.length === 0 &&
        <Typography sx={{ pl: '45px', fontSize: '14px', fontWeight: '500', color: '#b1b1b1' }}>No comment found!</Typography>
      }
      {cardComments.map((comment, index) =>
        <Box sx={{ display: 'flex', gap: 1, width: '100%', mb: 1.5 }} key={index}>
          ...
          <Box sx={{ width: 'inherit' }}>
            <Typography variant="span" sx={{ fontSize: '13px', fontWeight: 'bold', mr: 1 }}>
